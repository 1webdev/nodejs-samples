 ...
 */
ApiClient.trackDigests = function trackDigests(angularApp) {
...
ApiClient._internalPromise = ApiClient.prototype._internalPromise = BPromise;
 ...
 * projectId:
 */
ApiClient.prototype.setProject = ApiClient.prototype.projectId = function (projectId) {
 ...
 * options:
 */
ApiClient.prototype.adminAuthenticate = BPromise.method(function (credentials) {
    var url = this.baseUrl + 'auth/admin';

    if (this.options.project) credentials.projectId = this.options.project;

    debug('[ ] authenticate client: %s', url);
    return request.post(url).send(credentials).then(function (response) {
        ...
        debug('[' + chalk.green('o') + '] authenticate client: ' + chalk.cyan('%s'), response.body.user.username);
 ...
 * options:
 */
ApiClient.prototype.authenticate = BPromise.method(function (credentials) {
 ...
 *
 */
ApiClient.prototype.refreshAuth = BPromise.method(function refreshAuth(token) {
    token = token || this.auth.token;

    var url = this.baseUrl + 'api/info';

    debug('[ ] refresh auth information: %s', url);
    return request.get(url).set('x-access-token', token).then(function (response) {
        if (response.body.authUser) this.auth.user = response.body.authUser;

        if (response.body.authProfile) this.auth.profile = response.body.authProfile;

        debug('[' + chalk.green('o') + '] refresh auth information: ' + chalk.cyan('%s'), url);
 ...
 *
 */
ApiClient.prototype.hasPermissions = function authUserHasPermissions(levels, area, accessFor) {
    ...
    accessFor = accessFor || this.options.project || this.auth.user.tenant || this.auth.user.project;
    ...
    if (!permissions || !permissions[accessFor]) return false;
    // else:

    permissions = permissions[accessFor];

    if (permissions.permissions) permissions = permissions.permissions;
 ...
 */
ApiClient.prototype.newRequest = function (url, options) {
    // Set defaults
    ...
    var requestEvents = options.requestEvents || this.options.requestEvents;
    if ((typeof requestEvents === 'undefined' ? 'undefined' : _typeof(requestEvents)) === 'object') for (var event in requestEvents) {
...
[{ url: 'api/logsGlobal', props: ['logsGlobal'] }, { url: 'api/logsOrder', props: ['logsOrder'] }, { url: 'api/order/claim', props: ['claim'] }, { url: 'api/order/reject', props: ['reject'] }, { url: 'api/orders', props: ['order', 'orders'] }, { url: 'api/orderStatuses', props: ['status', 'statuses', 'orderStatuses'] }, { url: 'api/users', props: ['users', 'user'] }, { url: 'api', props: ['root', 'base', 'single', 'one'] }
// Save new registration ID
].forEach(function (row) {
    var getFunction = function getFunction() {
